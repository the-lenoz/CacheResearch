...
def loop_pattern(
    rng: random.Random, request_count: int, cache_size: int, key_space: int
) -> list[int]:
    working_set = max(1, cache_size + 1)
...
def hot_scan_pattern(
    ...
    block_size = max(16, cache_size * 2)
    scan_position = 0
    requests = []
...
def phase_change_pattern(
    ...
    working_set = max(1, cache_size // 2)
    requests = []

    for index in range(request_count):
        phase = index // phase_length
        first_key = (phase * working_set) % key_space
        requests.append((first_key + rng.randrange(working_set)) % key_space)
    return requests
...
def main() -> int:
    ...
    for pattern_name in selected_patterns:
        ...
        for cache_size in arguments.cache_sizes:
            key_space = max(cache_size + 2, cache_size * arguments.key_space_multiplier)
            for seed in arguments.seeds:
                ...
                filename = (
                    f"{pattern_name}_c{cache_size}_n{arguments.requests}_s{seed}.trace"
                )
                contents = (
